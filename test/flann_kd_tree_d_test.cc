...
using TestTree = flann::PhTreeMultiMap<DIM, T>;
...
TEST(PhTreeMMDTest, TestInsert) {
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        Id id(i);
        ASSERT_EQ(i, *tree.find(p, id));
        ASSERT_EQ(i / NUM_DUPL, *tree.find(p) / NUM_DUPL);
    }
}
...
TEST(PhTreeMMDTest, TestWindowQuery1) {
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        auto& x = *q;
        for (size_t j = 0; j < NUM_DUPL; j++) {
            ASSERT_EQ(i / NUM_DUPL, x / NUM_DUPL);
            ++q;
        }
        ASSERT_EQ(q, tree.end());
        n++;
...
TEST(PhTreeMMDTest, TestKnnQueryIterator) {
    ...
    while (q1 != tree.knn_end()) {
        ASSERT_NE(q1, tree.knn_end());
        ASSERT_NE(q2, tree.knn_end());
        ASSERT_EQ(q1, q2);
        q1++;
        ASSERT_NE(q1, q2);
        q2++;
